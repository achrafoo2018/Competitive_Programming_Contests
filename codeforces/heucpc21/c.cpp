...
void dfs(int i, int &priority, int parent){
	visited[i] = 1;
	priority = max(priority, st.query(i, n+1) + 1);
	for(int e : graph[i]){
		if(!visited[e]){
			dfs(e, priority, parent);
		}
	}
	st.update(i, priority);
	ans[i] = priority;
}
void solve(){
	cin >> n >> m;
	...
	For(i, n){
		...
		if(out[i] == 0){
			graph[i].PB(*stin.cbegin());
			stin.erase(*stin.begin());
		}
	}
	For(i, n){
