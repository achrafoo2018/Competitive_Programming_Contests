...
public:
	...
	Tree_Type query(int l, int r, int h){
		return query(l, r, h, 0, 0, size);
	}

 	Tree_Type query(int l, int r, int h, int x, int lx, int rx){
		if(lx >= r || l >= rx || tree[x].min > h){
			node ret = node();
			ret.max = 0;
			ret.min = INT_MAX;
			return ret;
		...
		if(lx >= l && rx <= r){
			if(tree[x].max <= h)
				...
				int m = (lx + rx) / 2;
				return merge2(query(l, r, h, 2*x+1, lx, m), query(l, r, h, 2*x+2, m, rx));
			}
		}
		int m = (lx + rx) / 2;
		return merge2(query(l, r, h, 2*x+1, lx, m), query(l, r, h, 2*x+2, m, rx));
	}
	Tree_Type merge2(node left, node right){
		node ret = node();
...
void solve(){
	...
	For(i, q){
		...
		if(l == r){
			cout << (arr[l] <= h ? arr[l] : 0) << endl;
			continue;
		}
		cout << st.query(l, r+1, h).max << endl;
	}
}
