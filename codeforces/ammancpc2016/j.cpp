...
public:
	...
	Tree_Type query(int l, int r){
 	...
 	Tree_Type query(int l, int r, int x, int lx, int rx){
		if(lx >= r || l >= rx) return vi(10);
		if(lx >= l && rx <= r) return tree[x];
		int m = (lx + rx) / 2;
		vi left = query(l, r, 2*x+1, lx, m);
		vi right = query(l, r, 2*x+2, m, rx);
		vi ans;
		For(i, 10) ans.PB(left[i]+right[i]);
		return ans;
	}
