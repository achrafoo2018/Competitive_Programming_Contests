...
public:
	...
	void init(int n){
		size = 1;
		while(size < n) size *= 2;
		tree.assign(size * 2, {INT_MIN, INT_MIN});
	}
...
void solve(){
	int n, q; cin >> n >> q;
	segment_tree<pii, pii> st;
	vpii arr(n);
	For(i, n){
		arr[i].second = (i+1);
	}
	st.build(arr);
	int ans = 0, winner = 1, best = 0;
	For(i, q){
		int x, p; cin >> x >> p;
		if(x == winner){
			best += p;
		...
		st.update(x-1, p);
		pii curr = st.query(0, n);
		if(curr.first > best){
			if(winner != curr.second){
				ans = i + 1;
				winner = curr.second;
			}
			best = curr.first;
