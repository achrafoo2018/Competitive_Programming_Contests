...
void solve(){
	...
	vector<pair<int, pair<int, pii>>> querries(q);
	For(i, q){
		cin >> querries[i].first >> querries[i].second.first >> querries[i].second.second.first;
		querries[i].first--;
		querries[i].second.first--;
		querries[i].second.second.second = i;
	}
	...
	vll ans(q);
	For(i, q){
		int d = querries[i].first;
		while(j <= d){
			int x = days[j].first;
			int y = days[j].second;
			ll p = min(arr[x], st.query(x, x+1) + y);
			st.update(x, p);
			j++;
		}
		int l = querries[i].second.first;
		int r = querries[i].second.second.first;
		ans[querries[i].second.second.second] = st.query(l, r);
	}
	For(i, q){
		cout << ans[i] << endl;
	}
